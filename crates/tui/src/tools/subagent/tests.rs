...
fn make_worker_spec(worker_id: &str, workspace: PathBuf) -> AgentWorkerSpec {
    AgentWorkerSpec {
        ...
        runtime_profile: crate::worker_profile::WorkerRuntimeProfile::for_role(
...
fn test_parse_spawn_request_rejects_out_of_range_max_depth() {
    // [pinvou3-fork] 上游 v0.8.65 把 MAX_SPAWN_DEPTH_CEILING 从 3 提到 8(决策③
    // runaway 护栏)。用常量取上限+1 构造越界值,稳健不写死,跟随合并后的 config。
